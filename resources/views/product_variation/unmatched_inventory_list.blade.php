@extends('master')
@section('title')
    Inventory | Unmatched Product | WMS360
@endsection
@section('content')
    <div class="content-page">
        ...
        <div class="content">
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="screen-option unmatched-breadcrumb">
                            <div>
                            ...
                            <div class="unmatched-csv-mb">
                                <form id="upload_csv" method="post" enctype="multipart/from-data">
                                    ...
                                    <div class="upload-csv-in-unmatched-list">
                                        <div class="mr-1">
                                            <a class="btn btn-default sample-csv-unmatched" href="{{URL::to('/check_quantity_sample.csv')}}"><i class="fa fa-download"></i> Sample CSV</a>
                                        </div>
                                        <div class="custom-file mr-1">
                                            <input type="file" class="custom-file-input" name="csvFile" id="csvFile" accept=".csv" required>
                                            <label class="custom-file-label unmatched-label" for="customFile">Upload CSV To Chek Qty</label>
                                        </div>
                                        <div>
                                            <button class="btn upload-csv-unmatched-btn">Submit</button>
                                        </div>
                                    </div>
                                </form>
                                {{-- <form id="upload_csv" method="post" enctype="multipart/from-data">
                            ...
                            <div class="tab-content p-50 product-content">
                                ...
                                <div id="threshold_product" class="tab-pane fade m-b-20">
                                    ...
                                    {{-- <div class="btn-group m-b-10">
                                        <input type="text" class="form-control" id="row-wise-search-1" onkeyup="threshold_search(this)"  name="threshold_value" value="5" placeholder="Search by your threshold value..."> --}}
                                        {{-- <button id="search_threshold" type="button" class="btn btn-purple btn-sm">search</button> --}}
                                    {{-- </div> --}}
                                    <div class="product-inner p-b-10">
                                        <div class="table-list"></div>
                                        <div class="row-wise-search table-terms">
                                            <input type="text" class="form-control mb-1" id="row-wise-search-1" onkeyup="threshold_search(this)" name="threshold_value" value="5" placeholder="Search by sku or threshold value...">
                                            <span class="threshold-validation"></span>
                                        </div>
                                    </div>
                                ...
                                <div id="negative_quantity" class="tab-pane @if($shelf_use == 0) active @else fade @endif m-b-20">
                                    <h5 class="text-center">Negative Quantity
                                        (<span>{{count($negative_quantity ?? [])}}</span>)</h5>

                                    <div class="product-inner p-b-10">
                                        <div class="table-list"></div>
                                        <div class="row-wise-search table-terms">
                                            <input class="form-control mb-1" id="negative-qty-row-wise-search" type="text" placeholder="Search....">
                                        </div>
                                    </div>

                                    <div class="table-responsive">
                                        <table id="datatable" class="table table-hover table-bordered negative-qty-table">
                                            <thead>
                                            <tr>
                                                <th>Image</th>
                                                <th>ID</th>
    ...
    <script>
        ...
        function threshold_search(e){
            // var threshold_value = $('input[name=threshold_value]').val();
            var threshold_value = $(e).val();
            if(threshold_value == ''){
                console.log('empty')
                $('.threshold-validation').text(null)
                return false;
            }
            // if (isNaN(threshold_value) || threshold_value == '' || threshold_value == 0 || threshold_value < 1) {
            if ( threshold_value == 0 || threshold_value < 1) {
                console.log('keyup')
                $('.threshold-validation').addClass('text-danger').text('Please input valid your threshold value')
                return false;
            }
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#negative-qty-row-wise-search").on("keyup", function(){
                let value = $(this).val().toLowerCase();
                $(".negative-qty-table tbody tr").filter(function () {
                    $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
                });
            })
            $("#row-wise-search-1").on("input", function () {
                let value = $(this).val().toLowerCase();
