...
class CWoocommerce implements EChannel
{
    ...
    public function updateQuantity($sku, $quantity, $change_reason,$channel = null,$ordered_quantity=null,$force_update=false)
    {
        ...
        if ($woo_variation_result != null){
            ...
            try{
                $logInsertData = $this->paramToArray(url()->full(),$change_reason,'Woocommerce',1,$sku,$variation_data,null,Auth::user()->name ?? 'Cron Job',$woo_variation_result->actual_quantity,$quantity,Carbon::now(),1,0);
                ...
                $logInsertData = $this->paramToArray(url()->full(),$change_reason,'Woocommerce',1,$sku,$variation_data,$woocom_put_result,Auth::user()->name ?? 'Cron Job',$woo_variation_result->actual_quantity,$quantity,Carbon::now(),'Error');
//                                continue;
